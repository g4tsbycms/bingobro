 ...
 * Bingo Brothers
 * Version 3.2
 * App forked from Karol Brennan development
 * Implements by  GMSystem
 * http://github.com/g4tsbycms
 */
// Dependencies
...
import { generateBingoBoard, getRandomBingoNumber, getBallDisplay, getLogoBallDisplay, getLanguageText } from "../utils.js";
...
class BingoGame extends Component {
	constructor(props) {
		...
		this.interval = null;
		this.chimes = [
			{ label: "Efeito 1", value: chime1 },
			{ label: "Efeito 2", value: chime2 },
			{ label: "Efeito 3", value: chime3 },
			{ label: "Efeito 4", value: chime4 },
			{ label: "Efeito 5", value: chime5 },
			{ label: "Efeito 6", value: chime6 },
			{ label: "Efeito 7", value: chime7 },
			{ label: "Efeito 8", value: chime8 },
			{ label: "Efeito 9", value: chime9 },
			{ label: "Efeito 10", value: chime10 },
			{ label: "Efeito 11", value: chime11 },
			{ label: "Efeito 12", value: chime12 },
			...
			if (this.state.evensOdds) {
				window.setTimeout(() => {
					this.say([
						...
						" ",
						" ",
						` mark every ${ball.number % 2 === 1 ? "número ímpar" : "nÚmero par"}`,
					]);
				}, 2000);
			} else {
				window.setTimeout(() => {
	...
	callBingoNumber = () => {
		...
		if (totalBallsCalled < totalPossibleBalls) {
			...
			this.say("É melhor alguém começat um novo bingo, porque estamos sem bolas para chamar!");
	...
	get resetConfirmationModalDisplay() {
		if (this.state.showResetModal === true) {
			return (
				<div>
					<div className="modal">
						<h4>Reiniciar o Bingo</h4>
						<p>VC TEM CERTEZA!?</p>
						<p className="red-text">
							ESSA PARTIDA <strong>NÃO</strong> VALERÁ MAIS (Caso não tenha terminado).
						</p>
						<p>
	 ...
	 */
	handleBugReport = () => {
		let subject = "Bingo Brothers bug report";
		let body = `Thank you for playing Bngo Brothers app and for taking the time to report a bug! Please describe what is happening to you so I may fix it ASAP.`;
		...
		window.open(`mailto:sams40tl@gmail.com?subject=${subject}&body=${body}`);
	};
						...
						<div className="col pattern-side shrink padding-xlg">
							...
							<div className="row no-wrap margin-bottom-lg justify-space-between white-text">
								<div className="col text-center margin-sm">
									...
									<div className="callNumber-text uppercase">Total</div>
								</div>
								<div className="col text-center margin-sm">
									<div className="callNumber notranslate">{this.previousCallDisplay}</div>
									<div className="callNumber-text uppercase">Última Bola</div>
								</div>
							</div>

							{/* -------- Pattern --------- */}
							{/*<Pattern pattern={this.state.selectedPattern} update={this.handleUpdatePattern} />*/}
							{/*Stylos de jogos removidos*/}
								{/* ----------- Removido opçao ganhos cartela ------------- 
					...
					<div className="container row justify-start align-start">
							...
							<section className="gameplay-controls">
								<div data-disabled={this.totalBallsCalled >= 75}>
									<button
										...
										{this.totalBallsCalled === 0 ? "Iniciar BINGO" : "Próxima BOLA"}
									</button>
									...
									<button
										...
										{this.state.running ? "Pausar" : "Modo Inteligente"}
									</button>
								</div>
								...
								<div data-visibility={this.state.displayBoardOnly === false ? "show" : "hide"}>
									...
									<div className="row no-wrap align-center justify-start">
										...
										<div className="col shrink text-center padding-vertical-lg padding-horizontal-lg">
											<div className="row no-wrap align-center slider" data-disabled={this.state.displayBoardOnly}>
												<div className="col shrink padding-right-lg white-text">Devagar</div>
												<div className="col">
													<Slider
														min={3500}
														max={30000}
												...
												<div className="col shrink padding-left-lg white-text">Rápido</div>
											</div>
										</div>
									</div>
									...
									<div className="row align-start justify-start">
										...
										<div className="col grow min-size-150 padding-horizontal-lg">
											...
											<div className="row no-wrap" data-visibility={this.speechEnabled === true ? "show" : "hide"}>
												<div className="col grow">HABILITE NARRATIVAS (Caso não ouvir).</div>
											</div>
										</div>
									</div>
									...
									<div
										...
										<div className="col shrink min-size-150 padding-vertical-md padding-horizontal-lg">
											<h6>Selecionar Narrador:</h6>
										</div>
										<div className="col grow min-size-150 padding-horizontal-lg">
											<Select
												className="select-input"
												placeholder="Esolher Narrador"
												menuPlacement="auto"
						...
						<div className="col grow min-size-350 padding-vertical-xxlg padding-horizontal-xxlg white-text">
							...
							<p className="wrap-text small-text">
								Bingo JM  was last updated on <strong>9/13/2022</strong>. Recent updates include:
							</p>
							<ul className="small-text padding-left-xlg">
								...
								<li>
									Added new{" "}
									<a href="https://gmsystemgamesnews.io" target="_blank" rel="noreferrer">
										outros games
									</a>{" "}
									game version!
								</li>
							...
							<p className="x-small-text wrap-text">
								Há uma nova versão do Let's Play Bingo sendo desenvolvida que incluirá atualizações de desempenho, bem como novas
recursos. Confira a edição beta com uma prévia de algumas das próximas melhorias visitando{" "}
								<a href="https://beta.gmsystemgames.io/">beta.gmsystemgames.io</a>!
							</p>
							<p className="x-small-text">
								Reportar bugs?{" "}
